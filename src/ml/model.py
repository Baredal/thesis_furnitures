...
# ─── v2 IMPROVEMENT: Deeper embedding head ───────────────────────────
# Replaces the single nn.Linear with:
#   Linear → BatchNorm → ReLU → Dropout → Linear
# This gives the projection layer enough capacity to learn non-linear
# compatibility mappings while regularizing via dropout + BN.
# ──────────────────────────────────────────────────────────────────────
def _make_embedding_head(in_features, embedding_dim, dropout=0.0):
    """
    v2 improvement: two-layer embedding head with BatchNorm, ReLU, and Dropout.
    v1 used a single nn.Linear — too few parameters to learn furniture
    compatibility beyond what the frozen backbone already provides.
    """
    hidden = max(in_features // 2, embedding_dim * 2)  # e.g. 512→256 for ResNet18
    layers = [
    ...
    if dropout > 0:
        layers.append(nn.Dropout(p=dropout))            # ← v2: regularization
    layers.append(nn.Linear(hidden, embedding_dim))
    return nn.Sequential(*layers)


# ─── v2 IMPROVEMENT: Freeze early backbone layers ────────────────────
# ImageNet conv1/layer1/layer2 detect edges, textures, and generic
# patterns that transfer well — no need to fine-tune them.
# Freezing saves memory, reduces overfitting risk, and lets the
# optimizer focus on furniture-specific features in layer3/layer4.
# ──────────────────────────────────────────────────────────────────────
def freeze_early_layers(backbone, prefixes=("0", "1", "2", "3", "4")):
    ...
    So prefixes ("0","1","2","3","4","5") freezes through layer2.
    """
    frozen = []
...
class SiameseResnet50(nn.Module):
    def __init__(self, embedding_dim=256, pretrained=True, dropout=0.0):
        ...
        # v2: deeper head with optional dropout
    ...
    def forward(self, x):
        ...
        x = torch.flatten(x, 1)
        x = self.embedding(x)
        x = nn.functional.normalize(x, p=2, dim=1)      # ← L2 norm kept
        return x
...
class SiameseResnet18(nn.Module):
    def __init__(self, embedding_dim=128, pretrained=True, dropout=0.0):
        ...
        # ── v2 IMPROVEMENT ────────────────────────────────────────────
        # v1: self.embedding = nn.Linear(512, embedding_dim)
        # v2: Linear(512→256) → BN → ReLU → Dropout → Linear(256→emb_dim)
